...
void start(double LL, double WW, double HH, double dx, double T)
{
    ...
    double ***cPrimeLast = allocate3DArray();
    ...
    while (t_total < T)
    {
        ...
        ptrs= updateC(c, cPrime, cPrimeLast, cNext, ux, uy, uz, dx, dt, E);
        c = ptrs.newC;
...
struct ptrStruct updateC(double ***c, double *** cPrime, double ***cPrimeLast, double *** cNext, double ***ux, double ***uy, double ***uz, double dx, double dt, int Euler)
{
    ...
    for (int i = 1; i < L-1; i++)
    {
        for (int j = 1; j < W-1; j++)
        {
            ...
            {            
                ...
                cNext[i][j][k] = c[i][j][k] + dt*(d)*cPrime[i][j][k] - (dt)*(.5)*cPrimeLast[i][j][k];
                if (cNext[i][j][k] < 0){
                    cNext[i][j][k] = 0;
                }
            }
