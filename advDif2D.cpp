...
void start(double LL, double HH, double dx, double T) {
    ...
    // processU(ux, uz, dx); // print initial conditions
...
double initializeC(int i, int k, double dx) {
    ...
    // Fix seed
    unsigned seed = 2023;
    // Create a Mersenne Twister engine with the given seed
    std::mt19937 mt(seed);
...
double initializeUz(int i, int k, double dx, double LL, double HH) {
    ...
    double z = coord[1];
    double uz;
    // uz = sin((2.0*N-1.0)*(2.0*x - LL/2.0)*(M_PI/LL)) * sin(M_PI*(z/HH));
    uz = Ub * cos((2 * M_PI * N) * ((x + LL / 2) / LL)) * sin((z * M_PI) / HH);
    return uz;
}
...
#pragma omp parallel for collapse(2)
    for (int i = 0; i < L; i++) {
        for (int k = 0; k < H; k++) {
            ...
            // if this branch is uncomment, we are imposing Neumann BC on top, as well,
            // so it will be a closed system
            /*
            if (k == 0) {
                c[i][k] = c[i][k + 1];
            } else if (k == H - 1) {
...
double updateC_cell(double **c, double **ux, double **uz, int i, int k, double dx) {
    ...
    // old way; it's bad, c is increasing if it were a closed system
    /*
...
void processU(double **ux, double **uz, double dx) {
    ...
    for (int k = H - 1; k > -1; k--) {
        for (int i = 0; i < L; i++) {
            ...
            std::cout << x << "," << z << "," << ux[i][k] << "," << uz[i][k] << std::endl;
        }
    }
}
